...
def load_graph(graph_nbr_train, node_nbr, logdir, logtime):
    ...
    for graph_ in range(graph_nbr_train):
        seed = graph_
        G = Graph_IM(nodes=node_nbr, edges_p=0.03, seed=seed)
        graph_dic[graph_] = G
        ...
        plt.xlabel("$k$")
        plt.ylabel("$p_k$")
        plt.savefig("./log/"+logdir+"/graphs/"+logtime+"_"+str(graph_)+".png")
...
for episode in range(training_setting["episodes"]):       #  one-step, adversary is a bandit
    ...
    # sample graph
    g_id_l = random.sample(graph_pool.keys(), 1)
    g_id = g_id_l[0]

    env.init_graph(g_id)
    nature_agent.init_graph(g_id)
    main_agent.init_graph(g_id)
    print(f"-----------this is -- {global_iter} iteration,  training in graph {g_id}")

    # sample node feature
    ft_id_l = random.sample(node_feat_pool.keys(), 1)
    ft_id = ft_id_l[0]
    ...
    # sample z
    hyper_id_l = random.sample(z_pool.keys(), 1)
    hyper_id = hyper_id_l[0]
