...
import random
import numpy as np
from layers import GraphAttentionLayer, GraphAttentionLayer_struc
import logging
...
class aggreMLP(nn.Module):
    def __init__(self, nlayer, n_hidden, input_dim, lr=None):
        ...
        if nlayer == 6:
            ...
            # output layer
            self.V = nn.Parameter(torch.zeros(size=(self.n_hidden * 2, 1)))
            nn.init.xavier_uniform_(self.V.data, gain=1.414)
...
class GAT_degree(nn.Module):
    def __init__(self, node_nbr, layer_tuple, nfeat, nhid_tuple, nfeat_s, nhid_s_tuple, alpha, nheads, mergeZ, mergeState, use_cuda, device, method):
        ...
        if self.model_v == "v1":
            ...
            self.hid = 10
            input_dims = 2
            self.ann = aggreMLP(6, self.hid, input_dims)
            ...
            self.hid = 10
            input_dims = nhid_tuple[1][-1] + nhid_s_tuple[1][-1]
            self.mlp = aggreMLP(3, self.hid, input_dims)
...
# model = GAT_degree(node_nbr, layer, nfeat=features_dim, nhid_tuple=hidden_dim, nfeat_s=nfeat_s, nhid_s_tuple=hidden_dim_s,alpha=alpha, nheads=nhead,
