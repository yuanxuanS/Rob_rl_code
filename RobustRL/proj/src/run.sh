      ...
      do
        ...
        do
          if [ $with_nature = "True" ]; then
            ...
            --logdir $logdir --logtime $runtime --seed-nbr $seeds --gamma $gamma --lr $lr >../$glb_logfile/$logdir/${runtime}_n.txt 2>../$glb_logfile/$logdir/${runtime}_n_error.txt &"
          else
            COMMAND="python3 -u train_adversary.py --nodes $nodes --budget $budget
            --graph-pool-nbr $graph_pn --train-graph-nbr $g_train_nbr --valid-graph-nbr $g_valid_nbr
            ...
            --train-episodes $episode --valid-episodes $valid_episodes
            --GAT-heads $nhead --hidden-dims $hid_dim --alpha $alpha
            --logdir $logdir --logtime $runtime --seed-nbr $seeds --gamma $gamma --lr $lr >../$glb_logfile/$logdir/$runtime.txt 2>../$glb_logfile/$logdir/${runtime}_error.txt &"
          fi
