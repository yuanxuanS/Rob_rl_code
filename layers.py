...
class GraphAttentionLayer_origin(nn.Module):
    """
    Simple GAT layer, similar to https://arxiv.org/abs/1710.10903
    """

    def __init__(self, in_features, out_features, dropout, alpha, concat=True, args=None):
        super(GraphAttentionLayer_origin, self).__init__()
    ...
    def forward(self, h, adj):
        print(f"h siez {h.size()}, W size {self.W.size()}")
        Wh = torch.mm(h, self.W)  # h.shape: (N, in_features), Wh.shape: (N, out_features)
        ...
        # print(f"final h_prime si {h_prime}")
        if self.concat:
